...
class CommonController extends Controller {
	...
	public function login(){
		...
		$usercount=User::where('username','=',$userdata['username'])->where('password','=',hash("sha256",$userdata['username'].$userdata['password'].'cookie'))->where('active','=','1')->count();
		if($usercount==1)
		{
			$user=User::where('username','=',$userdata['username'])->where('password','=',hash("sha256",$userdata['username'].$userdata['password'].'cookie'))->where('active','=','1')->first();
